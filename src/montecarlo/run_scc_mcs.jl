...
function run_scc_mcs(model::model_choice; 
    ...
    if model == DICE 
        ...
        base = get_dice_model(USG1, gas) # Need to set a scenario so the model can be built, but the scenarios will change in the simulation
        marginal = get_dice_model(USG1, gas)
    ...
    elseif model == FUND 
        ...
        base = get_fund_model(USG1, gas) # Need to set a scenario so the model can be built, but the scenarios will change in the simulation
        marginal = get_fund_model(USG1, gas)
        models = [base, marginal]
    ...
    elseif model == PAGE 
        ...
        base = get_page_model(USG1, gas)
        marginal = get_page_model(USG1, gas)
        models = [base, marginal]
    end
